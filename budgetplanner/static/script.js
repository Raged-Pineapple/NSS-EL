...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const pdfStatementInput = document.getElementById('pdfStatement');
    const pdfUploadStatus = document.getElementById('pdf-upload-status');

    if (uploadPdfBtn) {
        uploadPdfBtn.addEventListener('click', function() {
            const file = pdfStatementInput.files[0];
            if (!file) {
                pdfUploadStatus.innerHTML = `<div class="alert alert-warning">Please select a PDF file first.</div>`;
                return;
            }
            ...
            pdfUploadStatus.innerHTML = `<div class="alert alert-info">Uploading and processing... <i class="fas fa-spinner fa-spin"></i></div>`;
            ...
            .then(data => {
                if (data.success) {
                    pdfUploadStatus.innerHTML = `<div class="alert alert-success">Successfully processed! The page will now reload to update the list.</div>`;
                    setTimeout(() => {
                        location.reload();
                    }, 2000);
                } else {
                    pdfUploadStatus.innerHTML = `<div class="alert alert-danger">Error: ${data.error}</div>`;
                }
            })
            .catch(error => {
                console.error('Error:', error);
                pdfUploadStatus.innerHTML = `<div class="alert alert-danger">An unexpected error occurred: ${error.message}</div>`;
            });
        });
    }
    ...
    if (financialForm) {
        financialForm.addEventListener('submit', function(e) {
            ...
            const bankStatementSelect = document.getElementById('bankStatement');
            const selectedStatementFile = bankStatementSelect.value;
            ...
            const processFormData = (bsData) => {
                ...
                .then(response => {
                    if (response.success) {
                        ...
                        resultsContent.innerHTML = `<div class="alert alert-danger">Analysis failed: ${response.error}</div>`;
                        resultsSection.classList.remove('d-none');
                    }
                }).catch(error => {
                    console.error('Analysis error:', error);
                    resultsContent.innerHTML = `<div class="alert alert-danger">An unexpected error occurred during analysis.</div>`;
                    resultsSection.classList.remove('d-none');
                });
            };

            // If a processed statement is selected, fetch its data
            if (selectedStatementFile) {
                fetch(`/get_bank_statement/${selectedStatementFile}`)
                    .then(response => response.json())
                    .then(data => {
                        if (data.error) {
                            console.error('Error fetching bank statement:', data.error);
                            processFormData(null); // Proceed without bank data on error
                        } else {
                            processFormData(data); // Proceed with fetched bank data
                        }
                    })
                    .catch(error => {
                        console.error('Fetch error for bank statement:', error);
                        processFormData(null); // Proceed without bank data on error
                    });
            } else {
                processFormData(null); // No statement selected, proceed without it
            }
        });
    }
});
